...
#define     FLASH_START_ADDR        	0x08004000    //U盘的起始地址 前面留出16K的程序空间
...
int8_t STORAGE_Read_FS(uint8_t lun, uint8_t *buf, uint32_t blk_addr, uint16_t blk_len)
{
  /* USER CODE BEGIN 6 */
	if(lun == 0)
	{
		memcpy(buf,(uint8_t *)(FLASH_START_ADDR + blk_addr*STORAGE_BLK_SIZ),blk_len*STORAGE_BLK_SIZ); //yongkun
		return (USBD_OK);
	}
  return (USBD_FAIL);
  //return (USBD_OK);
...
int8_t STORAGE_Write_FS(uint8_t lun, uint8_t *buf, uint32_t blk_addr, uint16_t blk_len)
{
  /* USER CODE BEGIN 7 */
	...
	if(lun == 0)
	{
		...
		f.PageAddress = FLASH_START_ADDR + blk_addr*STORAGE_BLK_SIZ ;
		f.NbPages = blk_len;
